...
class DoubleFft1d(n: Int) {
    ...
    private var nw = 0
    private var nc = 0
    private lateinit var wtable: DoubleArray
    private lateinit var wtable_r: DoubleArray
    private lateinit var bk1: DoubleArray
    private lateinit var bk2: DoubleArray
     ...
     */
    @JvmOverloads
    fun complexForward(a: DoubleArray, offa: Int = 0) {
        if (n == 1) return
        when (plan) {
     ...
     */
    fun complexInverse(a: DoubleArray, scale: Boolean) {
     ...
     * @param scale if true then scaling is performed
     */
    fun complexInverse(a: DoubleArray, offa: Int, scale: Boolean) {
        if (n == 1) return
        when (plan) {
     ...
     */
    @JvmOverloads
    fun realForward(a: DoubleArray, offa: Int = 0) {
        if (n == 1) return
        when (plan) {
            Plans.SPLIT_RADIX -> {
                val xi: Double
     ...
     */
    @JvmOverloads
    fun realForwardFull(a: DoubleArray, offa: Int = 0) {
        val twon = 2 * n
        when (plan) {
            Plans.SPLIT_RADIX -> {
                realForward(a, offa)
     ...
     * @param scale if true then scaling is performed
     */
    fun realInverse(a: DoubleArray, scale: Boolean) {
     ...
     * @param scale if true then scaling is performed
     */
    fun realInverse(a: DoubleArray, offa: Int, scale: Boolean) {
        if (n == 1) return
        when (plan) {
            Plans.SPLIT_RADIX -> {
                ...
                if (scale) {
                    scale((n / 2).toDouble(), a, offa, false)
                }
            }
            Plans.MIXED_RADIX -> {
                var k = 2
     ...
     * @param scale if true then scaling is performed
     */
    fun realInverseFull(a: DoubleArray, scale: Boolean) {
     ...
     * @param scale if true then scaling is performed
     */
    fun realInverseFull(a: DoubleArray, offa: Int, scale: Boolean) {
        val twon = 2 * n
        when (plan) {
            Plans.SPLIT_RADIX -> {
    ...
    protected fun realInverse2(a: DoubleArray, offa: Int, scale: Boolean) {
        if (n == 1) return
        when (plan) {
            Plans.SPLIT_RADIX -> {
                val xi: Double
    ...
    fun cffti(n: Int, offw: Int) {
        ...
        factorize_loop@ while (true) {
            ...
            do {
                ...
                nf++
                wtable[offw + nf + 1 + fourn] = ntry.toDouble()
                nl = nq
                if (ntry == 2 && nf != 1) {
                    i = 2
                    while (i <= nf) {
                    ...
                    wtable[offw + 2 + fourn] = 2.0
                }
            } while (nl != 1)
            break@factorize_loop
        }
        wtable[offw + fourn] = n.toDouble()
        wtable[offw + 1 + fourn] = nf.toDouble()
        ...
        while (k1 <= nf) {
            ...
            while (j <= ipm) {
                i1 = i
                wtable[offw + i - 1 + twon] = 1.0
                wtable[offw + i + twon] = 0.0
                ld += l1
                fi = 0.0
                argld = ld * argh
                ii = 4
    ...
    fun rffti() {
        ...
        factorize_loop@ while (true) {
            ...
            do {
                ...
                if (nr != 0) continue@factorize_loop
                ++nf
                wtable_r[nf + 1 + twon] = ntry.toDouble()
                nl = nq
                if (ntry == 2 && nf != 1) {
                    i = 2
                    while (i <= nf) {
        ...
        wtable_r[twon] = n.toDouble()
        wtable_r[1 + twon] = nf.toDouble()
        argh = TWO_PI / n.toDouble()
        `is` = 0
        nfm1 = nf - 1
        l1 = 1
    ...
    private fun makect(nc: Int, c: DoubleArray, startc: Int) {
        var j: Int
        val nch: Int
        val delta: Double
        var deltaj: Double
    ...
    private fun bluestein_complex(a: DoubleArray, offa: Int, isign: Int) {
        val ak = DoubleArray(2 * nBluestein)
        var nthreads = ConcurrencyUtils.numberOfThreads
        if (nthreads > 1 && n > ConcurrencyUtils.threadsBeginN_1D_FFT_2Threads) {
            nthreads = 2
            if (nthreads >= 4 && n > ConcurrencyUtils.threadsBeginN_1D_FFT_4Threads) {
                nthreads = 4
            }
            val futures = arrayOfNulls<Future<*>?>(nthreads)
            var k = n / nthreads
    ...
    private fun bluestein_real_full(a: DoubleArray, offa: Int, isign: Int) {
        val ak = DoubleArray(2 * nBluestein)
        var nthreads = ConcurrencyUtils.numberOfThreads
        if (nthreads > 1 && n > ConcurrencyUtils.threadsBeginN_1D_FFT_2Threads) {
            nthreads = 2
    ...
    private fun bluestein_real_forward(a: DoubleArray, offa: Int) {
        val ak = DoubleArray(2 * nBluestein)
        var nthreads = ConcurrencyUtils.numberOfThreads
        if (nthreads > 1 && n > ConcurrencyUtils.threadsBeginN_1D_FFT_2Threads) {
            nthreads = 2
    ...
    private fun bluestein_real_inverse(a: DoubleArray, offa: Int) {
        val ak = DoubleArray(2 * nBluestein)
        if (n % 2 == 0) {
            ak[0] = a[offa] * bk1[0]
            ak[1] = a[offa] * bk1[1]
    ...
    private fun bluestein_real_inverse2(a: DoubleArray, offa: Int) {
        val ak = DoubleArray(2 * nBluestein)
        var nthreads = ConcurrencyUtils.numberOfThreads
        if (nthreads > 1 && n > ConcurrencyUtils.threadsBeginN_1D_FFT_2Threads) {
            nthreads = 2
    ...
    /*---------------------------------------------------------
      ...
      --------------------------------------------------------*/
    fun rfftf(a: DoubleArray, offa: Int) {
        if (n == 1) return
        var l1: Int
        var l2: Int
        var na: Int
    ...
    /*---------------------------------------------------------
      ...
      --------------------------------------------------------*/
    fun rfftb(a: DoubleArray, offa: Int) {
        if (n == 1) return
        var l1: Int
        var l2: Int
        var na: Int
    ...
    /*---------------------------------------------------------
      ...
      --------------------------------------------------------*/
    fun cfftf(a: DoubleArray, offa: Int, isign: Int) {
        var idot: Int
        var l1: Int
        var l2: Int
        var na: Int
    ...
    private fun cftfsub(n: Int, a: DoubleArray, offa: Int, ip: IntArray, nw: Int, w: DoubleArray) {
        if (n > 8) {
            if (n > 32) {
                cftf1st(n, a, offa, w, nw - (n shr 2))
                if (ConcurrencyUtils.numberOfThreads > 1 && n > ConcurrencyUtils.threadsBeginN_1D_FFT_2Threads) {
    ...
    private fun cftbsub(n: Int, a: DoubleArray, offa: Int, ip: IntArray, nw: Int, w: DoubleArray) {
        if (n > 8) {
            if (n > 32) {
    ...
    private fun bitrv2(n: Int, ip: IntArray, a: DoubleArray, offa: Int) {
        var j1: Int
        var k1: Int
        var l: Int
        var m: Int
    ...
    private fun bitrv2conj(n: Int, ip: IntArray, a: DoubleArray, offa: Int) {
        var j1: Int
        var k1: Int
        var l: Int
        var m: Int
    ...
    private fun bitrv216(a: DoubleArray, offa: Int) {
        val x1r: Double
        val x1i: Double
        val x2r: Double
        val x2i: Double
    ...
    private fun bitrv216neg(a: DoubleArray, offa: Int) {
        val x1r: Double
        val x1i: Double
        val x2r: Double
        val x2i: Double
    ...
    private fun bitrv208(a: DoubleArray, offa: Int) {
        val x1r: Double
        val x1i: Double
        val x3r: Double
        val x3i: Double
    ...
    private fun bitrv208neg(a: DoubleArray, offa: Int) {
        val x1r: Double
        val x1i: Double
        val x2r: Double
        val x2i: Double
    ...
    private fun cftf1st(n: Int, a: DoubleArray, offa: Int, w: DoubleArray, startw: Int) {
        var j0: Int
        var j1: Int
        var j2: Int
        var j3: Int
    ...
    private fun cftb1st(n: Int, a: DoubleArray, offa: Int, w: DoubleArray, startw: Int) {
        var j0: Int
        var j1: Int
        var j2: Int
        var j3: Int
    ...
    private fun cftrec4_th(n: Int, a: DoubleArray, offa: Int, nw: Int, w: DoubleArray) {
        var i: Int
        var idiv4: Int
        var m: Int
        var nthreads: Int
        ...
        idiv4 = 0
        m = n shr 1
        if (n > ConcurrencyUtils.threadsBeginN_1D_FFT_4Threads) {
            nthreads = 4
            idiv4 = 1
            m = m shr 1
        }
    ...
    private fun cftrec4(n: Int, a: DoubleArray, offa: Int, nw: Int, w: DoubleArray) {
        var isplt: Int
        var j: Int
        var k: Int
        var m: Int
    ...
    private fun cftleaf(n: Int, isplt: Int, a: DoubleArray, offa: Int, nw: Int, w: DoubleArray) {
        if (n == 512) {
    ...
    private fun cftmdl1(n: Int, a: DoubleArray, offa: Int, w: DoubleArray, startw: Int) {
        var j0: Int
        var j1: Int
        var j2: Int
        var j3: Int
    ...
    private fun cftmdl2(n: Int, a: DoubleArray, offa: Int, w: DoubleArray, startw: Int) {
        var j0: Int
        var j1: Int
        var j2: Int
        var j3: Int
    ...
    private fun cftfx41(n: Int, a: DoubleArray, offa: Int, nw: Int, w: DoubleArray) {
        if (n == 128) {
            cftf161(a, offa, w, nw - 8)
            cftf162(a, offa + 32, w, nw - 32)
            cftf161(a, offa + 64, w, nw - 8)
    ...
    private fun cftf161(a: DoubleArray, offa: Int, w: DoubleArray, startw: Int) {
        val wn4r: Double
        val wk1r: Double
        val wk1i: Double
        var x0r: Double
    ...
    private fun cftf162(a: DoubleArray, offa: Int, w: DoubleArray, startw: Int) {
        val wn4r: Double
        val wk1r: Double
        val wk1i: Double
        val wk2r: Double
    ...
    private fun cftf081(a: DoubleArray, offa: Int, w: DoubleArray, startw: Int) {
        val wn4r: Double
        var x0r: Double
        var x0i: Double
        var x1r: Double
    ...
    private fun cftf082(a: DoubleArray, offa: Int, w: DoubleArray, startw: Int) {
        val wn4r: Double
        val wk1r: Double
        val wk1i: Double
        var x0r: Double
    ...
    private fun cftf040(a: DoubleArray, offa: Int) {
        val x0r: Double
        val x0i: Double
        val x1r: Double
        val x1i: Double
    ...
    private fun cftb040(a: DoubleArray, offa: Int) {
        val x0r: Double
        val x0i: Double
        val x1r: Double
        val x1i: Double
    ...
    private fun cftx020(a: DoubleArray, offa: Int) {
        val x0r: Double
        val x0i: Double
        x0r = a[offa] - a[offa + 2]
    ...
    private fun cftxb020(a: DoubleArray, offa: Int) {
        val x0r: Double
        val x0i: Double
        x0r = a[offa] - a[offa + 2]
        x0i = a[offa + 1] - a[offa + 3]
    ...
    private fun cftxc020(a: DoubleArray, offa: Int) {
        val x0r: Double
        val x0i: Double
        x0r = a[offa] - a[offa + 2]
        x0i = a[offa + 1] + a[offa + 3]
    ...
    private fun rftfsub(n: Int, a: DoubleArray, offa: Int, nc: Int, c: DoubleArray, startc: Int) {
        var k: Int
        var kk: Int
        val ks: Int
        val m: Int
    ...
    private fun rftbsub(n: Int, a: DoubleArray, offa: Int, nc: Int, c: DoubleArray, startc: Int) {
        var k: Int
        var kk: Int
        val ks: Int
        val m: Int
    ...
    private fun scale(m: Double, a: DoubleArray, offa: Int, complex: Boolean) {
        val norm = 1.0 / m
        val n2: Int
        n2 = if (complex) {
            2 * n
        ...
        val nthreads = ConcurrencyUtils.numberOfThreads
        if (nthreads > 1 && n2 >= ConcurrencyUtils.threadsBeginN_1D_FFT_2Threads) {
            val k = n2 / nthreads
            val futures = arrayOfNulls<Future<*>?>(nthreads)
            for (i in 0 until nthreads) {
    ...
    companion object {
        ...
        private const val PI = 3.14159265358979311599796346854418516
        private const val TWO_PI = 6.28318530717958623199592693708837032
     ...
     */
    init {
        require(n >= 1) { "n must be greater than 0" }
        this.n = n
        if (!ConcurrencyUtils.isPowerOf2(n)) {
            if (getReminder(n, factors) >= 211) {
                ...
                bk1 = DoubleArray(2 * nBluestein)
                bk2 = DoubleArray(2 * nBluestein)
                ip = IntArray(
                    2 + ceil((2 + (1 shl (ln(nBluestein + 0.5) / ln(2.0)).toInt() / 2)).toDouble())
                        .toInt()
                )
                w = DoubleArray(nBluestein)
                ...
                wtable = DoubleArray(4 * n + 15)
                wtable_r = DoubleArray(2 * n + 15)
                cffti()
                rffti()
            }
        } else {
